...
type CustomActionItem = { id: string; planned_action_id: string; item_name: string; prepared_at: string | null; prepared_by: string | null; sort_order: number };
...
type TemplateCustomItem = { template_id: string; item_name: string; sort_order: number };
...
export default function ToolsPage() {
  ...
  const [customActionItems, setCustomActionItems] = useState<CustomActionItem[]>([]);
  const [templates, setTemplates] = useState<ChecklistTemplate[]>([]);
  const [templateItems, setTemplateItems] = useState<TemplateItem[]>([]);
  const [templateCustomItems, setTemplateCustomItems] = useState<TemplateCustomItem[]>([]);
  ...
  async function load() {
    ...
    setUserId(user.id);
    const [toolResult, profileResult, actionResult, actionToolResult, customActionItemResult, templateResult, templateItemResult, templateCustomItemResult] = await Promise.all([
      ...
      supabase.from("planned_actions").select("id,title,status,starts_at,ends_at,address").eq("tool_checklist_only", true).neq("status", "zruseno").order("starts_at", { ascending: false }).limit(180),
      ...
      supabase.from("tool_checklist_custom_items").select("id,planned_action_id,item_name,prepared_at,prepared_by,sort_order").order("sort_order"),
      ...
      supabase.from("tool_checklist_template_custom_items").select("template_id,item_name,sort_order").order("sort_order"),
    ]);
    const error = toolResult.error || profileResult.error || actionResult.error || actionToolResult.error || customActionItemResult.error || templateResult.error || templateItemResult.error || templateCustomItemResult.error;
    ...
    setCustomActionItems((customActionItemResult.data ?? []) as CustomActionItem[]);
    setTemplates((templateResult.data ?? []) as ChecklistTemplate[]);
    setTemplateItems((templateItemResult.data ?? []) as TemplateItem[]);
    setTemplateCustomItems((templateCustomItemResult.data ?? []) as TemplateCustomItem[]);
    setLoading(false);
  }
  ...
  const checklistActions = useMemo(() => {
    ...
    return actions.filter((action) =>
      actionTools.some((item) => item.planned_action_id === action.id)
      || customActionItems.some((item) => item.planned_action_id === action.id)
    ).sort((a, b) => {
  ...
  }, [actions, actionTools, customActionItems]);
  ...
  function customItemsForAction(actionId: string) {
    return customActionItems.filter((item) => item.planned_action_id === actionId).sort((a, b) => a.sort_order - b.sort_order);
  }
  function customItemsForTemplate(templateId: string) {
    return templateCustomItems.filter((item) => item.template_id === templateId).sort((a, b) => a.sort_order - b.sort_order);
  }
  ...
  function openNewChecklist() {
    ...
    setSelectedCustomItems(next.existingActionId ? customItemsForAction(next.existingActionId).map((item) => item.item_name) : []);
    setCustomItemDraft("");
    setToolPickerSearch("");
  ...
  function selectExistingAction(actionId: string) {
    ...
    setSelectedCustomItems(actionId ? customItemsForAction(actionId).map((item) => item.item_name) : []);
  }
  function applyTemplate(templateId: string) {
    ...
    setSelectedCustomItems(customItemsForTemplate(templateId).map((item) => item.item_name));
  }
  function toggleTool(toolId: string) {
    setSelectedToolIds((current) => current.includes(toolId) ? current.filter((id) => id !== toolId) : [...current, toolId]);
  }
  function addCustomItem() {
    const itemName = customItemDraft.trim().replace(/\s+/g, " ");
    if (!itemName) return;
    setSelectedCustomItems((current) => current.some((item) => item.toLocaleLowerCase("cs-CZ") === itemName.toLocaleLowerCase("cs-CZ"))
  ...
  async function saveChecklist(event: FormEvent) {
    ...
    if (selectedToolIds.length + selectedCustomItems.length === 0) { setMessage("Vyber nebo napiš alespoň jednu položku nářadí."); return; }
    ...
    const { error } = await supabase.rpc("save_tool_checklist_action", {
      ...
      custom_item_names: selectedCustomItems,
  ...
  async function toggleCustomPrepared(item: CustomActionItem) {
    if (!canManageTools) return;
    const preparedAt = item.prepared_at ? null : new Date().toISOString();
    const supabase = createClient();
    const { error } = await supabase.from("tool_checklist_custom_items")
      .update({ prepared_at: preparedAt, prepared_by: preparedAt ? userId : null }).eq("id", item.id);
    if (error) { setMessage(`Položku se nepovedlo změnit: ${error.message}`); return; }
    setCustomActionItems((current) => current.map((currentItem) => currentItem.id === item.id
      ? { ...currentItem, prepared_at: preparedAt, prepared_by: preparedAt ? userId : null } : currentItem));
  }
  ...
  async function saveCurrentAsTemplate(event: FormEvent) {
    ...
    const { error } = await supabase.rpc("save_tool_checklist_template", {
      ...
      custom_item_names: customItemsForAction(selectedChecklist.id).map((item) => item.item_name),
    });
  ...
  const selectedChecklistCustomItems = selectedChecklist ? customItemsForAction(selectedChecklist.id) : [];
  const selectedChecklistItemCount = selectedChecklistLinks.length + selectedChecklistCustomItems.length;
  const selectedPreparedCount = selectedChecklistLinks.filter((item) => item.prepared_at).length
    + selectedChecklistCustomItems.filter((item) => item.prepared_at).length;
  ...
  return <main className="tools-shell">
    ...
    <section className="checklists-card">
      ...
      {templates.length > 0 && <div className="template-row"><span>Uložené šablony:</span>{templates.map((template) => {
        const itemCount = templateItems.filter((item) => item.template_id === template.id).length
          + templateCustomItems.filter((item) => item.template_id === template.id).length;
        return <span className="template-chip" key={template.id}>{template.name}<small>{itemCount} ks</small>{canManageTools && <button title="Smazat šablonu" onClick={() => void deleteTemplate(template)}>×</button>}</span>;
      })}</div>}
      {loading ? <div className="empty compact-empty">Načítám checklisty…</div> : checklistActions.length === 0 ? <div className="empty compact-empty">Zatím tu není žádný checklist. Vytvoř ho pro první akci nebo použij uloženou šablonu.</div> : <div className="checklist-grid">{checklistActions.slice(0, 8).map((action) => {
        const links = linksForAction(action.id);
        const customItems = customItemsForAction(action.id);
        const itemCount = links.length + customItems.length;
        const prepared = links.filter((item) => item.prepared_at).length + customItems.filter((item) => item.prepared_at).length;
        const complete = itemCount > 0 && prepared === itemCount;
        return <button className={`checklist-card ${complete ? "complete" : ""}`} key={action.id} onClick={() => { setSelectedChecklist(action); setShowTemplateSave(false); }}><span className="checklist-top"><span className={`action-status ${action.status}`}>{actionStatusLabels[action.status]}</span><strong>{prepared}/{itemCount}</strong></span><span className="checklist-title">{action.title}</span><small>{actionDate(action.starts_at)}{action.address ? ` · ${action.address}` : ""}</small><span className="progress"><i style={{ width: `${itemCount ? (prepared / itemCount) * 100 : 0}%` }} /></span><span className="checklist-foot">{complete ? "✓ Všechno připraveno" : `Zbývá připravit ${itemCount - prepared} položek`}</span></button>;
      })}</div>}
    </section>
    ...
    {showChecklistForm && <div className="modal-backdrop" onClick={() => setShowChecklistForm(false)}><form className="modal checklist-editor" onSubmit={saveChecklist} onClick={(event) => event.stopPropagation()}><div className="modal-head"><div><h2>Příprava nářadí na akci</h2><p>Vyber akci, sestav seznam a případně si ho ulož jako šablonu.</p></div><button type="button" className="close" onClick={() => setShowChecklistForm(false)}>×</button></div><div className="editor-columns"><div className="editor-settings">
      ...
      <label>Použít šablonu<select value={checklistForm.templateId} onChange={(event) => applyTemplate(event.target.value)}><option value="">Bez šablony</option>{templates.map((template) => {
        const itemCount = templateItems.filter((item) => item.template_id === template.id).length
          + templateCustomItems.filter((item) => item.template_id === template.id).length;
        return <option key={template.id} value={template.id}>{template.name} ({itemCount} položek)</option>;
      })}</select></label>
      ...
      </div><div className="tool-picker"><div className="picker-head"><div><strong>Checklist nářadí</strong><small>{selectedToolIds.length + selectedCustomItems.length} vybráno</small></div><input value={toolPickerSearch} onChange={(event) => setToolPickerSearch(event.target.value)} placeholder="Hledat v evidenci…" /></div>
      <div className="custom-item-box"><strong>Přidat vlastní položku</strong><small>Nářadí nebo materiál, který není v evidenci</small><div><input value={customItemDraft} onChange={(event) => setCustomItemDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addCustomItem(); } }} placeholder="Např. prodlužovačka, vrtáky SDS…" /><button type="button" onClick={addCustomItem}>＋ Přidat</button></div>{selectedCustomItems.length > 0 && <div className="custom-item-chips">{selectedCustomItems.map((item) => <span key={item}>{item}<button type="button" aria-label={`Odebrat ${item}`} onClick={() => setSelectedCustomItems((current) => current.filter((currentItem) => currentItem !== item))}>×</button></span>)}</div>}</div>
      <div className="picker-list"><div className="picker-list-title">Nářadí z evidence</div>{pickerTools.map((tool) => <label className={`picker-item ${selectedToolIds.includes(tool.id) ? "picked" : ""}`} key={tool.id}><input type="checkbox" checked={selectedToolIds.includes(tool.id)} onChange={() => toggleTool(tool.id)} /><span><strong>{tool.name}</strong><small>{tool.inventory_number} · {statusLabels[tool.status]}{tool.current_holder_id ? ` u ${holderName(tool.current_holder_id)}` : ""}</small></span></label>)}</div></div></div><div className="modal-actions"><button type="button" className="secondary" onClick={() => setShowChecklistForm(false)}>Zrušit</button><button className="primary" disabled={saving}>{saving ? "Ukládám…" : checklistForm.existingActionId ? "Uložit checklist" : "Vytvořit akci a checklist"}</button></div></form></div>}

    {selectedChecklist && <div className="modal-backdrop" onClick={() => setSelectedChecklist(null)}><section className="modal checklist-detail" onClick={(event) => event.stopPropagation()}><div className="modal-head"><div><span className={`action-status ${selectedChecklist.status}`}>{actionStatusLabels[selectedChecklist.status]}</span><h2>{selectedChecklist.title}</h2><p>{actionDate(selectedChecklist.starts_at)}{selectedChecklist.address ? ` · ${selectedChecklist.address}` : ""}</p></div><button type="button" className="close" onClick={() => setSelectedChecklist(null)}>×</button></div><div className="detail-progress"><div><strong>{selectedPreparedCount} z {selectedChecklistItemCount} připraveno</strong><span>{selectedPreparedCount === selectedChecklistItemCount ? "Může se vyrazit" : `Zbývá ${selectedChecklistItemCount - selectedPreparedCount}`}</span></div><span className="progress large"><i style={{ width: `${selectedChecklistItemCount ? (selectedPreparedCount / selectedChecklistItemCount) * 100 : 0}%` }} /></span></div><div className="check-items">{selectedChecklistLinks.map((link) => { const tool = toolById(link.tool_id); if (!tool) return null; return <button disabled={!canManageTools} className={`check-item ${link.prepared_at ? "checked" : ""}`} key={link.id} onClick={() => void togglePrepared(link)}><span className="check-box">{link.prepared_at ? "✓" : ""}</span><span><strong>{tool.name}</strong><small>{tool.inventory_number} · {statusLabels[tool.status]}{tool.current_holder_id ? ` u ${holderName(tool.current_holder_id)}` : ""}</small></span><em>{link.prepared_at ? "Připraveno" : "Odškrtnout"}</em></button>; })}{selectedChecklistCustomItems.map((item) => <button disabled={!canManageTools} className={`check-item custom ${item.prepared_at ? "checked" : ""}`} key={item.id} onClick={() => void toggleCustomPrepared(item)}><span className="check-box">{item.prepared_at ? "✓" : ""}</span><span><strong>{item.item_name}</strong><small>Vlastní položka · není v evidenci</small></span><em>{item.prepared_at ? "Připraveno" : "Odškrtnout"}</em></button>)}</div>{canManageTools && <div className="template-save-area">{showTemplateSave ? <form onSubmit={saveCurrentAsTemplate}><input autoFocus required value={templateDraftName} onChange={(event) => setTemplateDraftName(event.target.value)} placeholder="Název nové šablony" /><button className="primary" disabled={saving}>Uložit šablonu</button><button type="button" className="secondary" onClick={() => setShowTemplateSave(false)}>Zrušit</button></form> : <button className="secondary" onClick={() => setShowTemplateSave(true)}>☆ Uložit tento seznam jako šablonu</button>}</div>}</section></div>}

...
export default function PlannedActionsPage() {
  ...
  const canManagePlanning = canManage("planned_actions");
  ...
  function canManageAction(action: ActionRow) {
    if (!canManagePlanning) return false;
  ...
  function canChangeActionStatus(action: ActionRow) {
    if (!canManagePlanning) return false;
    return canManageAction(action) || assignees.some((item) => item.planned_action_id === action.id && item.profile_id === currentUserId);
  }

  async function changeActionStatus(action: ActionRow, status: ActionStatus) {
    setUpdatingStatus(true);
    setMessage("");
    setSuccess("");
    const supabase = createClient();
    const { error } = await supabase.rpc("set_planned_action_status", {
      target_action_id: action.id,
      next_status: status,
    });
    setUpdatingStatus(false);
    if (error) {
      setMessage(`Stav práce se nepovedlo změnit: ${error.message}`);
      return;
    }
    setSelectedAction((current) => current?.id === action.id ? { ...current, status } : current);
    setSuccess(status === "hotovo" ? "Práce byla označena jako hotová." : `Stav práce byl změněn na „${statusLabels[status]}“.`);
    await load();
  }

  function isOverdue(action: ActionRow) {
    return !["hotovo", "zruseno"].includes(action.status) && new Date(action.ends_at).getTime() < Date.now();
  }
  ...
  return (
    <main className="planning-page">
      ...
      <section className={`planning-layout ${canManagePlanning ? "" : "read-only"}`} id="planning-calendar">
        {canManagePlanning && <form className="planning-panel planning-form" onSubmit={saveAction}>
          ...
          <label>Stav práce<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as ActionStatus })}>{(Object.keys(statusLabels) as ActionStatus[]).map((status) => <option key={status} value={status}>{statusLabels[status]}</option>)}</select></label>
        ...
        <section className="planning-calendar-column">
          <section className="planning-panel">
            ...
            <div className="planning-month">{calendarDays.map((date) => { const key = toDateKey(date); const items = actionsByDay.get(key) ?? []; const outside = date.getMonth() !== visibleMonth.getMonth(); return <button type="button" key={key} className={`${outside ? "outside" : ""} ${selectedDate === key ? "selected" : ""}`} onClick={() => selectDay(date)}><span className={key === todayKey ? "today" : ""}>{date.getDate()}</span><div>{items.slice(0, 6).map((item) => <i key={item.id} className={item.status === "hotovo" ? "completed" : isOverdue(item) ? "overdue" : ""} style={{ background: actionColor(item) }} title={`${item.title}: ${statusLabels[item.status]}`} />)}{items.length > 6 && <small>+{items.length - 6}</small>}</div></button>; })}</div>
          </section>
        ...
        <div className="planning-modal-backdrop" role="presentation" onMouseDown={() => setSelectedAction(null)}>
          <section className="planning-detail-modal" role="dialog" aria-modal="true" aria-labelledby="planning-detail-title" onMouseDown={(event) => event.stopPropagation()}>
            <header className="planning-detail-head">
              <div><span className={`planning-status-badge ${isOverdue(selectedAction) ? "overdue" : selectedAction.status}`}>{isOverdue(selectedAction) ? "Po termínu" : statusLabels[selectedAction.status]}</span><span className="planning-tag" style={{ color: typeColors[selectedAction.action_type] }}>{typeLabels[selectedAction.action_type]}</span><h2 id="planning-detail-title">{selectedAction.title}</h2><p>{statusLabels[selectedAction.status]}</p></div>
            ...
            <footer className="planning-detail-actions">
              ...
              {canChangeActionStatus(selectedAction) && selectedAction.status !== "rozpracovano" && selectedAction.status !== "hotovo" && <button type="button" className="progress" onClick={() => void changeActionStatus(selectedAction, "rozpracovano")} disabled={updatingStatus}>Začít práci</button>}
              {canChangeActionStatus(selectedAction) && selectedAction.status !== "hotovo" && <button type="button" className="complete" onClick={() => void changeActionStatus(selectedAction, "hotovo")} disabled={updatingStatus}>✓ Označit hotovo</button>}
              {canChangeActionStatus(selectedAction) && selectedAction.status === "hotovo" && <button type="button" className="reopen" onClick={() => void changeActionStatus(selectedAction, "planovano")} disabled={updatingStatus}>Vrátit k udělání</button>}

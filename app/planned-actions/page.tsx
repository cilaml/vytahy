...
export default function PlannedActionsPage() {
  ...
  async function load() {
    ...
    const [profilesResult, elevatorsResult, actionsResult, assigneesResult] = await Promise.all([
      ...
      supabase.from("planned_actions").select("id,title,action_type,status,starts_at,ends_at,address,contact_name,contact_phone,description,elevator_id").order("starts_at"),
    ...
    <main className="planning-page">
      ...
      <section className="planning-layout">
        ...
        <section className="planning-calendar-column">
          ...
          <section className="planning-panel planning-agenda">
            ...
            {loading ? <div className="planning-empty">Načítám…</div> : dayActions.length === 0 ? <div className="planning-empty">Na tento den zatím není nic naplánováno.</div> : <div className="planning-list">{dayActions.map((item) => <button type="button" className="planning-action-card" key={item.id} onClick={() => setSelectedAction(item)} aria-label={`Otevřít detail akce ${item.title}`}><i style={{ background: typeColors[item.action_type] }} /><div className="planning-time">{new Date(item.starts_at).toLocaleTimeString("cs-CZ", { hour: "2-digit", minute: "2-digit" })}<span>–</span>{new Date(item.ends_at).toLocaleTimeString("cs-CZ", { hour: "2-digit", minute: "2-digit" })}</div><div className="planning-action-summary"><span className="planning-tag" style={{ color: typeColors[item.action_type] }}>{typeLabels[item.action_type]}</span><h3>{item.title}</h3><p><strong>{actionElevator(item) || item.address || "Bez výtahu a adresy"}</strong></p><p>{namesForAction(item.id) || "Bez přiřazeného zaměstnance"}</p>{item.address && actionElevator(item) !== item.address && <p>{item.address}</p>}<span className="planning-open-detail">Zobrazit detail →</span></div></button>)}</div>}
          </section>
        </section>
      </section>
      ...
      {selectedAction && (
        <div className="planning-modal-backdrop" role="presentation" onMouseDown={() => setSelectedAction(null)}>
          <section
            className="planning-detail-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header className="planning-detail-head">
              <div>
                <span className="planning-tag" style={{ color: typeColors[selectedAction.action_type] }}>
                  {typeLabels[selectedAction.action_type]}
                </span>
                <h2 id="planning-detail-title">{selectedAction.title}</h2>
                <p>{statusLabels[selectedAction.status] ?? selectedAction.status}</p>
              </div>
            ...
            <div className="planning-detail-grid">
              <div>
                ...
                <strong>
                  {new Date(selectedAction.starts_at).toLocaleDateString("cs-CZ", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
                </strong>
                <p>
                  {new Date(selectedAction.starts_at).toLocaleTimeString("cs-CZ", { hour: "2-digit", minute: "2-digit" })}
                  {" – "}
                  {new Date(selectedAction.ends_at).toLocaleTimeString("cs-CZ", { hour: "2-digit", minute: "2-digit" })}
                </p>
              </div>
              <div>
                <span>Přiřazení</span>
                <strong>{namesForAction(selectedAction.id) || "Bez přiřazeného zaměstnance"}</strong>
              </div>
              <div>
                <span>Výtah</span>
                <strong>{actionElevator(selectedAction) || "Bez výtahu"}</strong>
              </div>
              <div>
                <span>Adresa</span>
                <strong>{selectedAction.address || "Bez uvedené adresy"}</strong>
              </div>
              <div>
                <span>Kontakt</span>
                <strong>{selectedAction.contact_name || "Bez kontaktu"}</strong>
                {selectedAction.contact_phone && <p>{selectedAction.contact_phone}</p>}
              </div>
            </div>

            <div className="planning-detail-note">
              <span>Poznámka</span>
              <p>{noteFromDescription(selectedAction.description) || "K této akci není uložená žádná poznámka."}</p>
            </div>

            <footer className="planning-detail-actions">
              {selectedAction.address && (
                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(selectedAction.address)}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  Otevřít navigaci
